# **📋 Sprint #1 Refactor Plan v8-FINAL: Policy Extraction & Dependency Injection**

*Production-ready implementation plan based on actual `spike_architecture.py` codebase analysis*

## **🎯 Epic Goals & Success Metrics**

**Epic:** Extract retry/circuit-breaker logic from `_execute_http_request` (lines 1524-1582) into PolicyExecutor pattern; enable full dependency injection for offline testing while preserving exact behavioral parity and backwards compatibility.

**Success Criteria:**
- ✅ **Zero sleep tests**: All unit tests run offline with deterministic execution (no `time.sleep()` calls)
- ✅ **Zero network tests**: Complete test isolation using policy injection
- ✅ **Exact behavioral parity**: Contract tests prove mathematical equivalence to current `@backoff` implementation
- ✅ **Performance improvement**: 70%+ reduction in per-call allocation overhead (eliminate `_Retryable` class + `_run()` function)
- ✅ **Backwards compatibility**: Zero breaking changes to public APIs
- ✅ **Error categorization**: Transport vs semantic distinction at source with factory methods
- ✅ **Policy flexibility**: Session-level + operation-level overrides with startup validation
- ✅ **Production readiness**: Thread-safety documented, edge cases handled

## **🔍 Codebase Reality Assessment**

### **Current State Analysis (`spike_architecture.py`):**
- **File size**: 1,831 lines with mature GoF pattern implementation
- **Retry hotspot**: Lines 1524-1582 in `CloudClient._execute_http_request`
- **Performance bottleneck**: Per-call allocation of `_Retryable` class (line 1535) and `_run()` function (line 1547)
- **CloudError usage**: @dataclass implementation (line 887), manual construction in 17 locations
- **Natural error boundary**: `_make_http_request` (transport) vs `_handle_response` (semantic) at lines 1583 and 1611
- **Architecture foundation**: CloudSession → CloudClient → AuthenticationProvider pattern exists
...
```python
# Lines 1535-1582: Performance-critical hot path
class _Retryable(Exception):  # ← PER-CALL ALLOCATION
...
def _run() -> Result[bytes, CloudError]:  # ← PER-CALL ALLOCATION
    # Current logic preserves: transport errors → semantic errors → retryable checks
```
...
```
CloudSession
├── PolicyRegistry (startup validation, immutable interface)
│   ├── DEFAULT_POLICY_NAME = "default_backoff"
│   ├── DefaultBackoffPolicy (preserves current @backoff behavior)
│   ├── NoRetryPolicy (deterministic testing)
│   └── Custom policies (extensible)
├── AuthenticationProvider (unchanged)
└── ProtocolHandlers (unchanged)

CloudClient
├── PolicyExecutor (injected Strategy) → execute(ctx, attempt_func)
...
**Duration:** 1.5 days
**Owner:** Developer
**Risk:** Medium (core retry logic replacement)
...
**Duration:** 1 day
**Owner:** Developer
**Risk:** Medium (core request flow modification)
...
class CloudClient:
    """Generic client with policy injection and emergency rollback support"""
    ...
    def __init__(self,
        ...
        # Log policy system status for operational awareness
        if self.use_policy_system:
            logger.info("🔧 Policy system ENABLED - using %s policy", policy_executor.name)
        else:
            logger.warning("🔧 Policy system DISABLED - using legacy @backoff decorator")
    ...
    def _execute_http_request(self,
                              ...
                              params: Dict[str, Any],
                              request_data: bytes) -> Result[bytes, CloudError]:
        """Central execution with rollback support for emergency situations"""

        if self.use_policy_system:
            return self._execute_with_policy(session, operation, url, headers, params, request_data)
        else:
            return self._execute_with_backoff(session, operation, url, headers, params, request_data)

    def _execute_with_policy(self,
                            session: Any,
                            operation: OperationModel,
                            url: str,
                            headers: Dict[str, str],
                            params: Dict[str, Any],
                            request_data: bytes) -> Result[bytes, CloudError]:
        """New policy-based execution path"""
        ...
        # Delegate to injected policy
        return self.policy_executor.execute(ctx, lambda: self._single_attempt(
            session, operation, url, headers, params, request_data
        ))

    def _execute_with_backoff(self,
                             session: Any,
                             operation: OperationModel,
                             url: str,
                             headers: Dict[str, str],
                             params: Dict[str, Any],
                             request_data: bytes) -> Result[bytes, CloudError]:
        """Original @backoff execution path - preserved for emergency rollback"""

        # IMPORTANT: This preserves the original implementation exactly
        # for emergency rollback during phased deployment
        ...
        def _run() -> Result[bytes, CloudError]:
            # Original implementation logic preserved
            io_res = self._single_attempt(session, operation, url, headers, params, request_data)
            if io_res.is_failure:
                err = io_res.error
                if err.retryable:
        ...
        try:
            return _run()
        except _Retryable as boom:
            logger.error("❌ Legacy backoff exhausted retries for %s", operation.name)
            return Result.failure(boom.ce)
        except requests.RequestException as boom:
            return Result.failure(
                CloudError.transport(
                    code=CloudErrorCode.NETWORK_ERROR,
                    message=str(boom),
                    details={"url": url, "exception": repr(boom)},
                )
            )
```
...
**Duration:** 1 day
**Owner:** Developer
**Risk:** Low (high-level orchestration changes)
...
class CloudSession:
    ...
    def __init__(self,
        ...
        # Production-ready default policy configuration
        ...
        # Log policy system status for operational visibility
...
### **Story V8-5: Test Infrastructure & Migration**
**Duration:** 1.5 days
**Owner:** Developer
**Risk:** Low (test infrastructure and validation)
...
- PolicyRegistry: Immutable after construction, safe for concurrent reads
- DefaultBackoffPolicy: Stateless design, safe for concurrent execution across threads
- NoRetryPolicy: Stateless design, safe for concurrent execution across threads
- CloudError: Immutable dataclass, safe for sharing between threads
- PolicyContext: Single-use per execution, not intended for sharing between threads

NOT THREAD-SAFE COMPONENTS:
- CloudClient: Not thread-safe (same as current implementation)
  * Each thread should use its own CloudClient instance
  * Session objects from AuthenticationProvider may have threading restrictions
- CloudSession: Not thread-safe (same as current implementation)
  * Session creation and client caching not synchronized
  * Each thread should use its own CloudSession instance
- FakeSleeper: Test utility only, not intended for concurrent use

CUSTOM POLICY REQUIREMENTS:
- Must be stateless OR manage their own synchronization
- sleep_func injection point allows thread-local sleep implementations
- PolicyContext objects are single-use per execution, never shared
- Policy.execute() may be called concurrently from different threads with different contexts

PRODUCTION DEPLOYMENT CONSIDERATIONS:
- Policy system enablement logged at startup for operational visibility
- Registry validation occurs once at construction time (thread-safe)
- Policy switching via environment variables requires application restart
- No runtime policy modification - use immutable registry patterns

TESTING THREAD SAFETY:
- Use separate FakeSleeper instances per test thread
- PolicyContext should be created fresh for each execution
- Avoid sharing policy executor instances unless confirmed stateless
"""
```
...
```python
# Feature flag support for emergency rollback
USE_POLICY_SYSTEM = os.getenv("PYICLOUD_USE_POLICY_SYSTEM", "true").lower() == "true"

# Add startup logging for operational awareness
logger.info("🚀 PyiCloud starting with policy system: %s",
           "ENABLED" if USE_POLICY_SYSTEM else "DISABLED (legacy @backoff)")

def _execute_http_request(self, ...):
    if USE_POLICY_SYSTEM:
        return self._execute_with_policy(...)
    else:
        return self._execute_with_backoff(...)  # Original implementation
```
...
def test_behavioral_equivalence():
    """Run same request with both approaches, verify identical results"""

    # Configure identical retry parameters
    policy_session = CloudSession()
    original_session = CloudSession()  # With feature flag disabled

    # Execute identical failing requests
    # Verify identical retry patterns, timing, and final results
...
### **Performance Metrics:**
- **Allocation reduction**: 70%+ decrease in retry path memory allocation
...
- **CPU overhead**: Policy execution overhead <10% vs direct method calls (aligned with test target)

### **Reliability Metrics:**
- **Test execution time**: Unit test suite runs in <5 seconds (vs current 30+ with auth/network)
- **Test determinism**: 100% reproducible test results with zero sleeps
...
- **Error creation verbosity**: 50%+ reduction with factory methods
- **Debug clarity**: Policy execution visible in structured logs
- **Extension ease**: New policy implementation in <50 lines
- **Test setup**: Development environment ready in <1 second vs current multi-minute auth flow

## **📅 Timeline & Dependencies**

| Story | Duration | Parallel Tasks | Risk Level | Validation |
|-------|----------|---------------|------------|------------|
| V8-1: Error Factories | 1 day | AST script development | Low | Factory usage coverage |
| V8-2: Policy Infrastructure | 1.5 days | Contract test development | Medium | Mathematical parity |
| V8-3: CloudClient Integration | 1 day | Performance benchmarks | Medium | Allocation measurement |
| V8-4: CloudSession Integration | 1 day | Integration test expansion | Low | Policy injection testing |
| V8-5: Test Infrastructure | 1.5 days | Documentation creation | Low | Offline execution validation |

**Total: 6 days** (includes built-in buffer time)
**Critical Path**: V8-1 → V8-2 → V8-3 → V8-4 → V8-5
**Parallel Work**: Performance benchmarking, documentation, test expansion

## **🎯 Definition of Done**

### **Functional Requirements:**
- ✅ All retry behavior mathematically equivalent to current @backoff implementation
- ✅ Zero breaking changes to public CloudSession/CloudClient APIs
- ✅ Policy injection enables complete offline testing (zero sleeps, zero network)
- ✅ Session-level and operation-level policy configuration with validation
- ✅ Factory methods reduce CloudError construction verbosity by 50%+

### **Quality Requirements:**
- ✅ 100% line coverage for all new policy infrastructure components
- ✅ Performance benchmarks prove 70%+ allocation reduction in retry hot path
- ✅ Contract tests validate exact mathematical parity with deterministic jitter
- ✅ Thread safety documented with clear guidelines for custom policy development
- ✅ Comprehensive startup validation catches all configuration errors

### **Documentation Requirements:**
- ✅ Policy development guide with examples and testing patterns
- ✅ Migration documentation with before/after code comparisons
- ✅ Performance tuning recommendations for different workload patterns
- ✅ Troubleshooting guide for policy execution and debugging

### **Production Readiness:**
- ✅ Feature flag support enables safe rollout and emergency rollback
- ✅ Structured logging provides complete policy execution visibility
- ✅ Registry validation prevents invalid configurations reaching production
- ✅ Memory profiling confirms allocation targets met under load

---

**This v8-FINAL plan represents the complete, implementation-ready roadmap for policy extraction with comprehensive codebase analysis, precise component design, detailed implementation tasks, robust testing strategy, and clear success criteria. Ready for immediate development.**
